...
if $RUNNING_GNOME; then
	...
	# install gnome
	sudo apt install -y gnome-browser-connector gnome-tweaks
	...
	sudo add-apt-repository universe -y && sudo add-apt-repository -y ppa:agornostal/ulauncher -y && sudo apt update && sudo apt -y install ulauncher
